...
def is_int?(str)
  str == "0" || str.to_i.to_s == str
end

def place(num, register)
  register[0] = num.to_i
end
...
def minilang(str)
  str.split.each do |substr|
    if is_int?(substr)
      place(substr, REGISTER)
    elsif substr.downcase == 'push'
      push(STACK, REGISTER)
    elsif substr.downcase == 'print'
      print(REGISTER)
    elsif substr.downcase == 'pop'
      pop(STACK, REGISTER)
    elsif substr.downcase == 'add'
      add(STACK, REGISTER)
    elsif substr.downcase == 'sub'
      sub(STACK, REGISTER)
    elsif substr.downcase == 'mult'
      mult(STACK, REGISTER)
    elsif substr.downcase == 'div'
      div(STACK, REGISTER)
    elsif substr.downcase == 'mod'
      mod(STACK, REGISTER)
    end
  end
end
